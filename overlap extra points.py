...
class Puzzle():
            ...
            with open(Filename) as f:
                ...
                for row in range(1, self.__GridSize  + 1):
                    for column in range(1, self.__GridSize+1):
                        ...
                        if notallowedsymbols != [""] and self.__GetCell(row,column).GetSymbol()== "-":
                            location.append((row,column))
                print(location)
                self.__Score = int(f.readline().rstrip())
                self.__SymbolsLeft = int(f.readline().rstrip())
                #x,location = self.CheckforMatchWithPattern(3,3)
        except:
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                AmountToAddToScore,row1,column1 = self.CheckforMatchWithPattern(Row, Column)
    ...
    def __GetCell(self, Row, Column):
        ...
        if Index >= 0:
            ...
            raise IndexError()
    def checkoverlap(self,row,column):
       pass
