...
class Puzzle():
    ...
    '''
    def All_rotation(self,pattern):
       spiral_list =  list(pattern)
        spiral_list[4] =
    '''
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):
                ...
                self.__Score = self.CheckforMatchWithALLPattern(3, 3)
            #if self.__Score>0:
             #   if self.CheckforMatchWithPattern(3, 3)==0:
              #      self.__Score-=10
            #if self.__Score > 10:
        ...
        for StartRow in range(Row + 2, Row - 1, -1):
            for StartColumn in range(Column - 2, Column + 1):
                ...
                try:
                    ...
                    pass
        return 0
    def CheckforMatchWithALLPattern(self, Row, Column):
        x=0
        for StartRow in range(Row + 2, Row - 1, -1):
            for StartColumn in range(Column - 2, Column + 1):
                ...
                try:
                    ...
                    for k in self.__AllowedPatterns:
                        for P in self.__AllowedSymbols:

                            if k.MatchesPattern(PatternString, P):
                                self.__GetCell(StartRow, StartColumn).AddToNotAllowedSymbols(P)
                                self.__GetCell(StartRow, StartColumn + 1).AddToNotAllowedSymbols(P)
                                self.__GetCell(StartRow, StartColumn + 2).AddToNotAllowedSymbols(P)
                                self.__GetCell(StartRow - 1, StartColumn + 2).AddToNotAllowedSymbols(P)
                                self.__GetCell(StartRow - 2, StartColumn + 2).AddToNotAllowedSymbols(P)
                                self.__GetCell(StartRow - 2, StartColumn + 1).AddToNotAllowedSymbols(P)
                                self.__GetCell(StartRow - 2, StartColumn).AddToNotAllowedSymbols(P)
                                self.__GetCell(StartRow - 1, StartColumn).AddToNotAllowedSymbols(P)
                                self.__GetCell(StartRow - 1, StartColumn + 1).AddToNotAllowedSymbols(P)
